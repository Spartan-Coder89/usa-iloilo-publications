...
<main>
  <figure id="post_thumbnail" style="background-image: url( <?php echo get_the_post_thumbnail_url( $current_single_post_id, 'full' ) ?> )">
  ...
  <article id="article">
    <?php 
    $news_cat = get_the_terms( $current_single_post_id, 'usa_tax_news_cat' ); 
    if ( !empty( $news_cat ) ) {
      echo '<div class="taxonomy" style="color: '. get_term_meta( $news_cat[0]->term_id, '_term_meta_news_cat', true ) .';">'. $news_cat[0]->name .'</div>';
    }
    ?>
    <h1>
